...
/**
 * PGFX AS2 压力测试控制器
 * 使用 GridFS 存储文件到 MongoDB
 */
@RestController
@RequestMapping("/api/pgfxas2")
public class PGFXAS2Controller {

    private static final Logger logger = LoggerFactory.getLogger(PGFXAS2Controller.class);

    /**
     * 注入 PG EDI 专用的 GridFsTemplate
     * bucket名称: PG.DATAHUB_EDI_FILE
     */
    @Autowired
    @Qualifier("pgEdiGridFsTemplate")
        ...
        try {
            ...
            for (File file : files) {
                ...
                try {
                    ...
                    metadata.put("contentType", getFileExtension(fileName)); // 文件后缀

                    // 使用 GridFsTemplate 上传文件
                    ...
                    failCount++;
                    logger.error("GridFS存储失败: {}, 错误: {}", fileName, e.getMessage());
                }
            }
            ...
            data.put("collections", "PG.DATAHUB_EDI_FILE.files / PG.DATAHUB_EDI_FILE.chunks");
    ...
    /**
     * 查询导入记录
     * GET /api/pgfxas2/records
     */
    @GetMapping("/records")
    public ResponseEntity<Map<String, Object>> getRecords() {
        Map<String, Object> response = new HashMap<>();
        // 可以通过 GridFsTemplate 查询文件列表
        // 具体实现根据需求补充
        response.put("success", true);
        response.put("message", "查询功能待实现");
        return ResponseEntity.ok(response);
    }
